...
def obfuscation_machine(use_unicode=False, identifier_length=1):
    ...
    if use_unicode:
        ...
        big_list = list(map(chr, range(0x1F600, 0x1F644)))
        # big_list += list(map(chr, range(9986, 10160)))
        # big_list += list(map(chr, range(128640, 128704)))
        max_chars = len(big_list) # Ought to be enough for anybody :)
        combined = []
...
def obfuscate(module, tokens,replacement_length = 5,  name_generator=None, table=None):
    """
    ...
    *module* must be the name of the module we're currently obfuscating
    ...
    if not name_generator:
        ignore_length = True
        name_generator = obfuscation_machine(use_unicode=True, identifier_length=identifier_length)
